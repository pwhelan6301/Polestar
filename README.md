...
- The GitHub Actions workflow (`.github/workflows/azure-static-web-apps-…yml`) now performs two deployments on every push to `main`: the Static Web App upload and a follow-up job that logs into Azure using a service-principal secret (`AZURE_CREDENTIALS`) and runs `func azure functionapp publish DBPipeline --python`. This mirrors the manual Flex Consumption deployment process and ensures `host.json`/`function_app.py` land at the Function App root. Keep the service-principal credentials current, or rerun `func azure functionapp publish DBPipeline --python` locally from `api/` if you need to redeploy outside CI.
